...
public class InvoiceExtractionService {
    ...
    @Value("${claude.retry.enabled:true}")
    private boolean retryEnabled;

    @Value("${claude.retry.max-attempts:2}")
    private int maxRetryAttempts;

    @Value("${claude.retry.delay-seconds:30}")
    ...
    public List<InvoiceItemDTO> extractInvoiceItems(MultipartFile pdfFile) throws IOException {
        ...
        String jsonResponse = generateContentWithRetry(pdfBytes);
        return parseInvoiceItems(jsonResponse);
    }

    private String generateContentWithRetry(byte[] pdfBytes) {
        if (!retryEnabled) return generateContent(pdfBytes);
        int attempt = 1;
        while (attempt <= maxRetryAttempts) {
            try {
                return generateContent(pdfBytes);
    ...
    private String generateContent(byte[] pdfBytes) {
        ...
        if (response == null || !response.containsKey("content")) {
            throw new RuntimeException("Failed to extract invoice items from PDF via Claude API.");
        }
        List<Map<String, Object>> content = (List<Map<String, Object>>) response.get("content");
        if (content == null || content.isEmpty()) {
            throw new RuntimeException("Empty content returned from Claude API.");
        }
    ...
    public InvoiceSaveResult saveToInventory(List<InvoiceItemDTO> items, MultipartFile pdfFile) {
        ...
        for (InvoiceItemDTO dto : items) {
            ...
            if (!violations.isEmpty()) {
                ...
                failureMessages.add("Validation failed for item '" + dto.getName() + "': " + errorMsg);
                failureCount++;
                continue;
            }

            // HIGH-9: Validate business-critical numeric fields before touching the DB
            if (dto.getPurchaseRate() == null || dto.getPurchaseRate().compareTo(BigDecimal.ZERO) <= 0) {
                failureMessages.add("Item '" + dto.getName() + "': purchaseRate must be greater than zero (got: "
                        + dto.getPurchaseRate() + "). Skipping.");
                failureCount++;
                continue;
            }
            if (dto.getQuantity() == null || dto.getQuantity().compareTo(BigDecimal.ZERO) <= 0) {
                failureMessages.add("Item '" + dto.getName() + "': quantity must be greater than zero (got: "
                        + dto.getQuantity() + "). Skipping.");
                failureCount++;
                continue;
            }

            // HIGH-9: Warn when Claude returns a non-statutory GST rate (convertGst will default to 18%)
            if (dto.getGstPercentage() != null) {
                java.util.Set<Integer> validGstRates = java.util.Set.of(0, 5, 12, 18, 28);
                if (!validGstRates.contains(dto.getGstPercentage())) {
                    failureMessages.add("Item '" + dto.getName() + "': GST rate " + dto.getGstPercentage()
                            + "% is not a valid Indian statutory rate (0, 5, 12, 18, 28); defaulted to 18%.");
                    log.warn("Invalid GST rate {}% for item '{}', defaulting to 18%", dto.getGstPercentage(), dto.getName());
                }
            }
            ...
            try {
                ...
                log.error("Unexpected error saving invoice item '{}': {}", dto.getName(), e.getMessage(), e);
                failureMessages.add("Failed to save item '" + dto.getName() + "': " + e.getMessage());
                failureCount++;
            }
        }
