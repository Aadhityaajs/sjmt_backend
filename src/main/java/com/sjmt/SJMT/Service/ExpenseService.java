...
public class ExpenseService {
    ...
    @Transactional
    public ExpenseResponseDTO updateExpense(Integer expenseId, ExpenseRequestDTO dto, String username) {
        ExpenseEntity entity = expenseRepository.findByExpenseIdAndStatus(expenseId, RecordStatusEnum.ACTIVE)
                .orElseThrow(() -> new EntityNotFoundException("Expense not found with ID: " + expenseId));
    ...
    private void mapRequestToEntity(ExpenseRequestDTO dto, ExpenseEntity entity, ExpenseCategoryEntity category) {
        if (dto.isGstApplicable()) {
            if (dto.getGstPercentage() == null || dto.getGstPercentage().getValue() <= 0) {
                throw new IllegalArgumentException("GST percentage must be provided when GST is applicable");
            }
            if (dto.getCgstAmount() == null && dto.getIgstAmount() == null) {
                throw new IllegalArgumentException("GST amounts must be provided when GST is applicable");
            }
        } else {
            if (dto.getGstPercentage() != null && dto.getGstPercentage().getValue() > 0) {
                throw new IllegalArgumentException("GST percentage should be 0 or null when GST is not applicable");
            }
            if ((dto.getCgstAmount() != null && dto.getCgstAmount().compareTo(BigDecimal.ZERO) > 0) ||
                (dto.getSgstAmount() != null && dto.getSgstAmount().compareTo(BigDecimal.ZERO) > 0) ||
                (dto.getIgstAmount() != null && dto.getIgstAmount().compareTo(BigDecimal.ZERO) > 0)) {
                throw new IllegalArgumentException("GST amounts must be 0 when GST is not applicable");
            }
        }
