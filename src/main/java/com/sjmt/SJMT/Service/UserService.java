...
public class UserService {
     ...
     */
    @Transactional
    public UserResponse updateUserRoleAndPrivileges(Integer userId, UserRoleEnum role, PrivilegesEnum privileges, String currentUsername) {
        ...
        if (user.getUsername().equals(currentUsername) && 
            (role != UserRoleEnum.ADMIN || privileges != PrivilegesEnum.UPDATE)) {
            throw new RuntimeException("You cannot remove your own ADMIN role or privileges");
        }
     ...
     */
    @Transactional
    public UserResponse updateUserStatus(Integer userId, UserStatusEnum status, String currentUsername) {
        ...
        if (user.getUsername().equals(currentUsername) && 
            (status == UserStatusEnum.BLOCKED || status == UserStatusEnum.DELETED)) {
            throw new RuntimeException("You cannot disable, block, or delete your own account");
        }
            
        if (user.getStatus() == UserStatusEnum.DELETED) {
            throw new RuntimeException("Cannot update status of a deleted user");
        }
        
        if (status == UserStatusEnum.DELETED) {
            throw new RuntimeException("Cannot set user status to DELETED. Use the delete user endpoint instead.");
        }
