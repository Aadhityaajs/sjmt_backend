...
//        // Get user entity
    ...
    public void verifyEmail(String token) {
        ...
        // NEW LOGIC: If there is a plain temp password, send it now
        if (user.getTempPasswordPlain() != null) {
            emailService.sendTemporaryPasswordEmail(
                user.getEmail(), 
                user.getUsername(), 
                user.getTempPasswordPlain()
            );
            
            // Clear the plain password after sending for security
