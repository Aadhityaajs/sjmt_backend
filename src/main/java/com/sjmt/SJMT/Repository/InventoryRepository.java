...
public interface InventoryRepository extends JpaRepository<InventoryEntity, Integer> {
    ...
    /**
     * MEDIUM-4: Supplier-level aggregation.
     * Returns [supplierId, supplierName, lineCount, totalQty, totalValue] per supplier.
     */
    @Query("SELECT i.supplier.id, i.supplier.name, COUNT(i), " +
           "COALESCE(SUM(i.quantity), 0), COALESCE(SUM(i.quantity * i.purchaseRate), 0) " +
           "FROM InventoryEntity i " +
           "WHERE i.status = :status AND i.supplier IS NOT NULL " +
           "GROUP BY i.supplier.id, i.supplier.name " +
           "ORDER BY i.supplier.name")
    List<Object[]> getSupplierStockSummary(@Param("status") RecordStatusEnum status);
}