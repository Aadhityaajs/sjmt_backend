...
public interface CategoryRepository extends JpaRepository<CategoryEntity, Integer> {
    ...
    @Query("SELECT s.category.id, COUNT(s) FROM SubCategoryEntity s WHERE s.status = :status GROUP BY s.category.id")
    List<Object[]> countActiveSubCategoriesGroupByCategoryId(RecordStatusEnum status);

    @Query("SELECT i.category.id, COUNT(i) FROM InventoryEntity i WHERE i.status = :status GROUP BY i.category.id")
    List<Object[]> countActiveInventoryItemsGroupByCategoryId(RecordStatusEnum status);
}